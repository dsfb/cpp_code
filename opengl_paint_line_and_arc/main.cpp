...
{
	if (button != GLUT_LEFT_BUTTON || paint.IsFinish) return;
...
void MenuClickEvent(int itemId)
{
	...
	case MENU_ITEM_UNDO:
		if (paint.CurrentMode == Mode::Line ||
			(paint.CurrentMode == Mode::Arc && paint.size() > 1))
			paint.Back();
		glutPostRedisplay();
		break;
...
void DrawArc(Element& AB, float aABF, float aBOF, float r, Point o)
{
	...
	for (float a = 0; a <= aBOF; a += 0.5f)
	{
		if (aAB > 0 && aABF > 0) // AB(UP, LEFT)
		{
			if (isLeftSide)
				gamma = a + shift;
			else gamma = a - shift; // 4 and false ->0/
		}
		else if (aAB > 0 && aABF < 0) // AB(UP, RIGHT)
		{
			if (isLeftSide)
				gamma = (360. - shift) - a;
			else gamma = (360. + shift) - a; // 2 and false /0
		}
		else if (aAB <= 0 && aABF < 0) // AB(DOWN, LEFT)
		{
			if (isLeftSide)
				gamma = (360. - a) + shift;
			else gamma = (360. - a) - shift; // 4 and false 0/
		}
...
void Display()
{
	...
	if (paint.CurrentMode == Mode::Line)
	{
		...
		else glVertex2f(paint.back().a->x, paint.back().a->y);
...
Kiedy zrobiłe: robiłem Fri May 12 10:54:24 2017
